...
export class OpenCvJsAdapter implements OpenCvAdapter {
    ...
    public findContours(
        ...
        const grayImage = this.createGrayscaleImage(image);

        try {
            return this.findContoursOnSingleChannelImage(
                grayImage
            );
        } finally {
            grayImage.delete();
    ...
    private createGrayscaleImage(
        ...
        if (
            ...
            throw new Error(
                "OpenCV runtime does not support RGBA to grayscale conversion."
            );
        }
    ...
    private findContoursOnSingleChannelImage(
        image: OpenCvMat
    ): readonly OpenCvContour[] {
